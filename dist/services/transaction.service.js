...
export class TransactionService {
    static async ingestSms(params) {
        ...
        let matchedInvoice;
        const pendingInvoices = await InvoiceRepository.findPendingByMerchantAndAmount(params.merchantId, parsed.amount);
        if (pendingInvoices.length > 0) {
            const invoice = pendingInvoices[0];
            const lockRes = await TransactionRepository.verifyAndLock(params.merchantId, parsed.trxId, parsed.amount, invoice.invoice_id);
            if (lockRes.success) {
                await InvoiceRepository.updateStatus(invoice.invoice_id, 'PAID', parsed.trxId, parsed.provider.toLowerCase());
