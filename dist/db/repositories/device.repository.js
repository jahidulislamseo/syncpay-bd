    ...
    static async updateHeartbeat(deviceIdOrToken, telemetry) {
        ...
        if (supabase && isSupabaseConfigured()) {
            ...
            try {
                ...
                if (telemetry?.device_model)
                    updatePayload.device_model = telemetry.device_model;
                if (telemetry?.android_version)
                    updatePayload.android_version = telemetry.android_version;
                if (telemetry?.device_name)
                    updatePayload.device_name = telemetry.device_name;
                ...
                if (deviceList.length > 0) {
                    return deviceList.map((d) => {
                        ...
                        return {
                            ...
                            sim_slots: localDev?.sim_slots ? (typeof localDev.sim_slots === 'string' ? (() => { try {
                                return JSON.parse(localDev.sim_slots);
                            }
                            catch (_) {
                                return null;
                            } })() : localDev.sim_slots) : (typeof d.sim_slots === 'string' ? (() => { try {
