export interface ParsedMfsResult {
  success: boolean;
  provider: 'bKash' | 'Nagad' | 'Rocket' | 'Upay' | 'DBBL' | 'BRAC' | 'IslamiBank' | 'CityBank' | 'UNKNOWN';
  trxId?: string;
  amount?: number;
  sender?: string;
  fee?: number;
...
export class MfsParser {
  ...
  private static parseBkash(text: string): ParsedMfsResult {
  ...
  private static parseNagad(text: string): ParsedMfsResult {
  ...
  private static parseRocket(text: string): ParsedMfsResult {
  ...
  private static parseUpay(text: string): ParsedMfsResult {
   ...
   */
  private static parseDBBL(text: string): ParsedMfsResult {
   ...
   */
  private static parseBRAC(text: string): ParsedMfsResult {
   ...
   */
  private static parseIslamiBank(text: string): ParsedMfsResult {
   ...
   */
  private static parseCityBank(text: string): ParsedMfsResult {
  ...
  public static parse(senderAddress: string, body: string): ParsedMfsResult {
    const combined = `${senderAddress} ${body}`.toUpperCase();

    // MFS Wallets (priority order)
    if (combined.includes('BKASH')  || combined.includes('16247')) return this.parseBkash(body);
    if (combined.includes('NAGAD')  || combined.includes('16167')) return this.parseNagad(body);
    if (combined.includes('ROCKET') || combined.includes('16216')) return this.parseRocket(body);
    if (combined.includes('UPAY')   || combined.includes('16268')) return this.parseUpay(body);

    // Banks
    if (combined.includes('DBBL') || combined.includes('DUTCH-BANGLA') || combined.includes('DUTCHBANGLA')) return this.parseDBBL(body);
    if (combined.includes('BRAC BANK') || combined.includes('BRACBANK')) return this.parseBRAC(body);
    if (combined.includes('IBBL') || combined.includes('ISLAMI BANK') || combined.includes('ISLAMIBANK'))   return this.parseIslamiBank(body);
    if (combined.includes('CITY BANK') || combined.includes('CITYBANK') || combined.includes('CBTXN'))      return this.parseCityBank(body);

    // Auto-detect by body keywords
    if (body.includes('bKash')  || body.includes('TrxID'))  return this.parseBkash(body);
    if (body.includes('Nagad')  || body.includes('TxnID'))  return this.parseNagad(body);
    if (body.toLowerCase().includes('upay'))                 return this.parseUpay(body);
    if (body.includes('IBBL'))                               return this.parseIslamiBank(body);
    if (body.includes('BRAC'))                               return this.parseBRAC(body);
